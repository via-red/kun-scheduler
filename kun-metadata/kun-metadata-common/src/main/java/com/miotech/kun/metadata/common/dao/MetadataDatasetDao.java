...
public class MetadataDatasetDao {
    ...
    public List<DatabaseBaseInfo> getDatabases(List<Long> dataSourceIds) {
        ...
        String whereClause = "kmd.database_name is not null ";
        if (CollectionUtils.isNotEmpty(dataSourceIds)) {
            String inClausePlaceholders = "and kmd.datasource_id in (" + StringUtils.repeat("?", ",", dataSourceIds.size()) + ") ";
            whereClause = whereClause + inClausePlaceholders;
        }
    ...
    public DatasetBasicSearch searchDatasets(BasicSearchRequest request) {
        ...
        String limitSql = "limit 1 offset 0";
        sql += limitSql;
        ...
        return dbOperator.fetchOne(sql, rs -> {
            ...
            while (rs.next()) {
                ...
                datasetBasicInfos.add(basic);
            }

            page.setDatasets(datasetBasicInfos);
            return page;
        }, String.format("%%s%", request.getKeyword().toUpperCase()), request.getKeyword());
    }
    ...
    public DatasetBasicSearch fullTextSearch(DatasetSearchRequest request) {
        ...
        return dbOperator.fetchOne(sql, rs -> {
            List<DatasetBasicInfo> datasets = Lists.newArrayList();
            while (rs.next()) {
                DatasetBasicInfo datasetBasicInfo = new DatasetBasicInfo();
                setDatasetBasicField(datasetBasicInfo, rs);
                datasets.add(datasetBasicInfo);
            }

            pageResult.setDatasets(datasets);
            return pageResult;
        }, pstmtArgs.toArray());
    }
    ...
    private void setDatasetBasicField(DatasetBasicInfo datasetBasicInfo, ResultSet rs) throws SQLException {
        ...
        highWatermark.setTime(NumberUtils.toDouble(DateTimeUtils.fromTimestamp(rs.getTimestamp(1)).toEpochSecond()));
    ...
    public DatasetDetail getDatasetDetail(Long id) {
        ...
        return dbOperator.fetchOne(sql + whereClause + groupByClause, rs -> {
            DatasetDetail dataset = new DatasetDetail();
            if (rs.next()) {
                setDatasetBasicField(dataset, rs);
                Watermark lowWatermark = new Watermark();
                lowWatermark.setTime(NumberUtils.toDouble(DateTimeUtils.fromTimestamp(rs.getTimestamp("low_watermark")).toEpochSecond()));
    ...
    public DatasetFieldPageInfo searchDatasetFields(Long id, DatasetColumnSearchRequest searchRequest) {
        ...
        List<DatasetFieldInfo> datasetFields = dbOperator.fetchOne(sql, rs -> {
